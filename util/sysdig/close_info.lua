...
description = "Show the info about file close syscall"
short_description = "file close info"
category = "malware"
...
function on_init()
	-- print("pid %d",g_pid)
	local filter = string.format(" fd.type=file and evt.type=close and evt.dir=< and (proc.pid >= %d or proc.apid = %d  or proc.name contains %s)", g_pid, g_pid, g_pname)
	-- print(filter)
...
function on_event()
	...
	dst = string.format("%s: path=%s, ret=%s", evt.field(fevtype),path,evt.field(fraw_arg_res))
