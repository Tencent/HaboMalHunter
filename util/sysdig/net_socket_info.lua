...
function on_init()
	-- print("pid %d",g_pid)
	local filter = string.format(" evt.type=socket and evt.dir=> and (proc.pid >= %d or proc.apid = %d  or proc.name contains %s)", g_pid, g_pid, g_pname)
	-- print(filter)
...
function on_event()
	...
	dst = string.format("%s: %s",evt.field(fevtype),evt.field(fevtinfo))
